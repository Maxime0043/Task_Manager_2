...
describe(`PUT ${urlWithoutId}/:id`, () => {
  ...
  it("should return 400 if the client id is not an Integer", async () => {
  ...
  it("should return 400 if the userRole name already exists", async () => {
  ...
  it("should return 200 if the userRole is updated", async () => {
    const res = await supertest(app).put(url).set("Cookie", cookie).send({
      name: "test",
      label: "Test",
    });
