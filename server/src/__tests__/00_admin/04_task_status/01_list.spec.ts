...
import initDB, { populateTaskStatus } from "../../init-db";
import User from "../../../db/models/user";

const url = "/api/v1/admin/task_status";
...
describe(`GET ${url}`, () => {
  beforeAll(async () => {
    ...
    // Populate database with taskStatus
  ...
  it("should return 200 and the list of the taskStatus", async () => {
    const res = await supertest(app).get(url).set("Cookie", cookie).query({
      limit: 2,
    });

    expect(res.status).toBe(200);
    expect(res.body.taskStatus).toHaveLength(2);
  });

  it("should return 200 and the list of the taskStatus with the correct order", async () => {
    const res = await supertest(app)
      .get(url)
      .set("Cookie", cookie)
    ...
    expect(res.status).toBe(200);
    expect(res.body.taskStatus[0].label).toBe("Status 9");
  });
  ...
  it.each([
    ...
    "should return 200 and the list of the taskStatus with the correct order - %p",
    async (query) => {
      const res = await supertest(app)
        .get(url)
        .set("Cookie", cookie)
