...
describe(`GET ${url}/:id`, () => {
  ...
  it("should return 404 if the taskStatus does not exist", async () => {
  ...
  it("should return 200 and the taskStatus details", async () => {
    const res = await supertest(app).get(url).set("Cookie", cookie);

    expect(res.status).toBe(200);
    expect(res.body.taskStatus.name).toBe("not_started");
    expect(res.body.taskStatus.label).toBe("Not Started");
  });
});
