...
describe(`PUT ${urlWithoutId}/:id`, () => {
  ...
  it("should return 400 if the projectStatus name already exists", async () => {
  ...
  it("should return 200 if the projectStatus is updated", async () => {
    const res = await supertest(app).put(url).set("Cookie", cookie).send({
      name: "test",
      label: "Test",
      color: "#0000FF",
    ...
    expect(projectStatus.color).toBe("#0000FF");
  });
});
