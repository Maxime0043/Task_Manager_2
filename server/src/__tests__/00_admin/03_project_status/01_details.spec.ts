...
describe(`GET ${url}/:id`, () => {
  ...
  it("should return 404 if the projectStatus does not exist", async () => {
  ...
  it("should return 200 and the projectStatus details", async () => {
    const res = await supertest(app).get(url).set("Cookie", cookie);

    expect(res.status).toBe(200);
    expect(res.body.projectStatus.name).toBe("not_started");
    expect(res.body.projectStatus.label).toBe("Not Started");
  });
});
